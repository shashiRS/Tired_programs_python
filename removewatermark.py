...
def get_inner_and_outer_masks(mask):
    print mask
    inner_mask = binary_erosion(binary_erosion(binary_dilation(mask,out=None)))
    inner_pixel_count = np.count_nonzero(mask)
    #inner_mask = mask
...
def display_and_output_image(name, im):
    cv2.imshow(name,im)
    file_name = os.path.join( "/home/halk/Videos", name + ".jpg")
...
def main():
    ...
    https://www.architecture.com/image-library/RIBApix/licensed-image/poster/balintore-castle-angus-the-entrance-front/posterid/RIBA65186.html
    """
    im = cv2.imread("966ff285-9d65-4af5-9bf6-b81cf01782f3.jpeg")
    print (im.shape)
